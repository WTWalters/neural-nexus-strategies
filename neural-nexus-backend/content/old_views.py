...
class BlogPostViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def track_analytics(self, request, slug=None):
        post = self.get_object()
        analytics, created = BlogAnalytics.objects.get_or_create(blog_post=post)
        ...
        if time_on_page:
            ...
            if is_bounce:
                analytics.bounce_rate = (
                    (analytics.bounce_rate * analytics.return_visits + 1)
                    / (analytics.return_visits + 1)
                )

            analytics.return_visits += 1

            if request.data.get('shared', False):
                analytics.social_shares += 1

            analytics.save()

        return Response(BlogAnalyticsSerializer(analytics).data)
